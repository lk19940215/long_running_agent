...
省次数方法：一次 2x opus 4.6 thinking 请求，需要引导，会话的能力才能产生最大的作用。所以应该确保一次请求使用产生1000W 甚至5000W 的token量 (包括缓存命中），才能省次数。
唯一问题：多轮对话后，会有点卡。
核心原则：尽可能的对话，从而尽可能多的输出。
解释: Claude Code有提问工具 "AskQuestion", 触发时可以让用户输入，用户输入时，可以指导模型来处理不同的事务。因此，在Cursor中按如下方法可以实现尽可能多的对话（不知道工具名）从而节省次数：
方法如下：
1 新开会话时，输入任何内容，末尾附加："（ 回答 or 执行 ），然后AskQuestion". 
2 触发 Cusror 提问，输入内容后，需要再次 末尾附加： "（ 回答 or 执行 ），然后AskQuestion". 
