...
// 📌 定位：工具编排——将多个 tool_use 分区为并行/串行批次执行
// 🔗 调用链：partitionToolCalls → runToolsConcurrently / runToolsSerially → toolExecution.runToolUse
//
// 核心概念：
//   - partitionToolCalls()：连续只读工具合并并行批次，写操作独占串行批次
//   - runToolsConcurrently / runToolsSerially：执行策略
