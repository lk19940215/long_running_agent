...
// 📌 定位：流式工具执行器——模型输出的同时开始执行工具，减少总延迟
// 🔗 调用链：流式解析 tool_use → addTool 入队 → runToolUse（toolExecution）并行或串行
//
// 核心概念：
//   - addTool()：模型流式输出时收到 tool_use block 立即排队执行
//   - 并发安全工具立即并行；非并发安全工具阻塞队列
//   - siblingAbortController：并行子进程出错时取消兄弟任务
