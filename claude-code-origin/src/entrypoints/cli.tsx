...
// 📌 定位：CLI 命令行入口，整个应用的「大门」
//
// 🔗 调用链：bootstrap-entry.ts → 本文件 → main.tsx → REPL
//
// 🎯 核心职责：根据命令行参数做路由分发
//    - 11 个快速路径（fast path）：特殊命令，提前 return，避免加载完整 CLI
//    - ⭐ 1 个核心路径（最后几行）：加载 main.tsx → cliMain()，启动完整 REPL
//
// 💡 设计理念：所有 import 都是动态的 await import()
//    每条路径只加载自己需要的模块，--version 甚至零额外模块加载
//
// 📖 背景：本仓库是 Claude Code v2.1.88 源码泄露后的社区还原版
//    Anthropic 2026年3月在 npm 发布时意外包含了 source map，暴露了 50万+ 行源码
//    Claude Code 并未官方开源，Anthropic 已发起 DMCA 投诉
//
// ⚠️ 关于 MACRO 全局变量：
//    代码中直接使用的 MACRO.VERSION 等，在原版中是 Bun bun:bundle 编译时注入的
//    还原版在 bootstrap-entry.ts 中用 globalThis.MACRO = {...} 运行时模拟
//
// ============================================================================
// 📋 快速路径分类一览（全部在核心路径之前，命中即 return）
//
// ┌─────────────────────────────────────────────────────────────────────────┐
// │ 第一类：用户日常使用                                                      │
// │  ① --version / -v    查看版本号（零模块加载，最快返回）                      │
// │  ⑪ --worktree --tmux 在 tmux 中启动 git worktree 隔离分支                │
// ├─────────────────────────────────────────────────────────────────────────┤
// │ 第二类：后台/无头运行（CI/CD、自动化场景）                                   │
// │  ④ --daemon-worker        守护进程派生的 worker 子进程                     │
// │  ⑥ daemon                 长驻守护进程管理                                │
// │  ⑦ ps/logs/attach/kill    后台会话管理                                    │
// │  ⑧ new/list/reply         模板任务                                       │
// │  ⑨ environment-runner     BYOC（自有算力）无头运行器                       │
// │  ⑩ self-hosted-runner     自托管运行器，轮询 Anthropic API                │
// ├─────────────────────────────────────────────────────────────────────────┤
// │ 第三类：IDE/浏览器集成                                                     │
// │  ③ --claude-in-chrome-mcp Chrome 扩展的 MCP 服务端                        │
// │  ③ --chrome-native-host   Chrome 原生消息主机                             │
// │  ③ --computer-use-mcp     计算机操作 MCP（屏幕/鼠标/键盘）                  │
// ├─────────────────────────────────────────────────────────────────────────┤
// │ 第四类：远程/桥接                                                          │
// │  ⑤ remote-control / rc    将本地暴露为远程环境                              │
// ├─────────────────────────────────────────────────────────────────────────┤
// │ 第五类：内部评测（Anthropic 内部使用，外部构建时通过 feature() 消除）          │
// │  ② --dump-system-prompt   导出系统提示词                                   │
// └─────────────────────────────────────────────────────────────────────────┘
//
// 为什么需要快速路径？
// → 性能！main.tsx 有 140+ 个 import 和大量初始化逻辑。如果每次都加载完整 CLI，
//   即使 `claude --version` 也要等好几秒。快速路径让特殊命令各走各的，互不拖累。
// ============================================================================

import { feature } from 'bun:bundle';

// ============================================================================
// 【环境准备】模块被 import 时立即执行，都是源码原有的逻辑
// ============================================================================

// 禁用 corepack 的自动 pin 行为，避免它往用户的 package.json 里塞 yarnpkg 配置
process.env.COREPACK_ENABLE_AUTO_PIN = '0';

// 远程容器环境（CCR）下，限制子进程最大堆内存为 8GB（容器总共 16GB）
if (process.env.CLAUDE_CODE_REMOTE === 'true') {
  const existing = process.env.NODE_OPTIONS || '';
  process.env.NODE_OPTIONS = existing ? `${existing} --max-old-space-size=8192` : '--max-old-space-size=8192';
}

// 消融实验基线（Anthropic 内部 A/B 测试用）：关闭一系列高级特性来对比效果
// 必须放在这里而不是 init.ts，因为 BashTool 等会在模块加载时就捕获环境变量
...
async function main(): Promise<void> {
  ...
  // ========== 快速路径 3：Chrome 浏览器集成（MCP 服务 / 原生消息主机） ==========
  ...
  // ========== 快速路径 4：守护进程 worker（由 supervisor 进程派生） ==========
  ...
  // ========== 快速路径 5：远程控制 / 桥接模式 ==========
  // 将本地机器作为远端桥接环境，支持多种别名：remote-control / rc / remote / sync / bridge
  if (feature('BRIDGE_MODE') && (args[0] === 'remote-control' || args[0] === 'rc' || args[0] === 'remote' || args[0] === 'sync' || args[0] === 'bridge')) {
    ...
    // 鉴权检查必须在 GrowthBook 特性门控之前——没有 auth 的话 GrowthBook 没有用户上下文
    const {
  ...
  // ========== 快速路径 11：tmux + worktree 模式 ==========
  // 在加载完整 CLI 之前就 exec 进 tmux，实现 git worktree 隔离
  ...
  // ========== 参数修正 ==========
  // 将常见的错误用法 --update / --upgrade 重定向到 update 子命令
  if (args.length === 1 && (args[0] === '--update' || args[0] === '--upgrade')) {
    process.argv = [process.argv[0]!, process.argv[1]!, 'update'];
  }

  // --bare 模式：提前设置 SIMPLE 标志，让后续模块加载时就能感知到精简模式
  if (args.includes('--bare')) {
    process.env.CLAUDE_CODE_SIMPLE = '1';
  }

  // ========================================================================
  // ⭐⭐⭐ 核心路径：没有匹配到任何特殊参数，加载并启动完整的 CLI ⭐⭐⭐
  //
  // 这里才是 99% 用户会走到的路径：
  //   1. 捕获用户在 CLI 启动前的早期输入
  //   2. 动态加载 main.tsx（整个应用的编排中心）
  //   3. 执行 cliMain()，最终启动 REPL 交互界面
  // ========================================================================
  const {
...
// 模块被 import 时自动执行 main()
// void 操作符表示不关心返回的 Promise（fire-and-forget）
void main();
